...
#[test]
fn every_legacy_ruleset_keeps_its_door_free_fixture_and_rejects_new_operations() {
    for ruleset in [
        "two-room-v1",
        "portal-sight-v2",
        "observer-scene-v3",
        ...
        assert!(engine
            ...
            .visible_cells
            .iter()
            .all(|c| c.door.is_none()));
        engine.enable_wizard().unwrap();
        let before = engine.state(ActorId(1)).unwrap();
        assert!(wizard(&mut engine, "door", "door 1 3 1 0 closed").is_err());
        assert!(send(
            &mut engine,
            "action",
            Command::Act {
        ...
        .is_err());
        assert_eq!(before, engine.state(ActorId(1)).unwrap());
        if ruleset == "travel-v5" {
            let target = &before
                .observation
                .visible_cells
                .iter()
                ...
                .unwrap()
                .key;
            assert!(engine.travel_route(ActorId(1), target).is_ok());
        }
    }
}

#[test]
fn shadowcasting_and_previous_door_rules_survive_resume_and_rewind() {
    for (ruleset, diagonal_visible) in [("doors-v6", false), ("shadowcasting-v7", true)] {
        let dir = tempdir().unwrap();
        let path = dir.path().join("perception.json");
        drop(Engine::open(&path, Scenario::two_room(42)).unwrap());
        let mut archive: serde_json::Value =
            serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();
        archive["ruleset"] = ruleset.into();
        ...
        assert_eq!(
            before
                ...
                .visible_cells
                .iter()
                .any(|c| c.position == Position { x: 1, y: -1, z: 0 }),
            diagonal_visible
        );
        assert!(!before
            .observation
            .visible_cells
            .iter()
            .any(|c| c.position == Position { x: 2, y: 0, z: 0 }));
        drop(engine);
        let mut engine = Engine::open(&path, Scenario::two_room(0)).unwrap();
        ...
        let archive: serde_json::Value =
            serde_json::from_slice(&std::fs::read(path).unwrap()).unwrap();
        assert_eq!(archive["ruleset"], ruleset);
    }
}
