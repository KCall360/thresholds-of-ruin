...
Protocol **6** requires `place_hint` on each disclosed `visible_cells` entry.
...
New games use `place-hints-v4`. Save format remains **3**: the new rules version
...
use protocol 6. Normal games are never implicitly promoted to wizard mode.
