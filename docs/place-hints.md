...
Protocol **10** retains the protocol-6 requirement for `place_hint` on each disclosed `visible_cells` entry.
...
New games use `material-rims-v10`. Save format remains **3**: the new rules version
...
use protocol 10. Normal games are never implicitly promoted to wizard mode.
